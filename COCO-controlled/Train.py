...
from smart_tune import get_intervention
...
def train(mode, main, spurious, p_correct, trial, p_main = 0.5, p_spurious = 0.5, n = 2000,
    ...
    elif mode == 'smart-tune':
        names = get_intervention(num_both, num_just_main, num_just_spurious, num_neither)
        print(names)
    ...
    elif mode in ['minimal-tune', 'rrr-tune', 'gs-tune', 'cdep-tune', 'simple-tune', 'smart-tune']:
        model, optim_params = get_model(mode = 'tune', parent = parent_transfer)
    else:
