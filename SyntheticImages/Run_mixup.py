...
def eval(mode, n, p, n_neutral):
    ...
    train(model, loss, X_train_aug, Y_train_aug, X_val_aug, Y_val_aug, "Models/new", mixup = True)
...
if __name__ == "__main__":
    ...
    result = np.zeros((n_runs, 3 + num_out))
    c = 0
    for mode in modes:
        for n in n_array:
            for p in p_array:
                config_avg = np.zeros((num_out))
                ...
                c += 1
                np.savetxt("Run_mixup.csv", np.round(result, 3), fmt = "%f", delimiter=",", header = "mode, n, p, Original - Test, Original - Neutral, Mixup - Test, Mixup - Neutral")
