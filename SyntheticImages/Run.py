...
from Datasets import StandardDataset, my_dataloader
...
def train_base(X_train, X_val, X_test, X_neutral, Y_train, Y_val, Y_test, Y_neutral, name, model_load = None, \
    ...
    # Train the model
    model = train_model(model, dataloaders, criterion, optimizer, scheduler, num_epochs = num_epochs, mixup = mixup, mixup_alpha = mixup_alpha, verbose = False)
...
def train_augmented(num_epochs = 25):
    ...
    # Train and evaluate
    model = train_base(X_train, X_val, X_test, X_neutral, Y_train, Y_val, Y_test, Y_neutral, 'model_augmented', model_load = 'model_original', num_epochs = num_epochs)
...
def train_mixup(num_epochs = 25):
    ...
    # Train and evaluate
    model = train_base(X_train, X_val, X_test, X_neutral, Y_train, Y_val, Y_test, Y_neutral, 'model_mixup', model_load = 'model_original', num_epochs = num_epochs, mixup = True)
